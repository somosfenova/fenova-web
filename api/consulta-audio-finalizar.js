const {
  env,supabaseHeaders,cleanText,verifyUploadSession,trackingFields,notifyTelegram
}=require('./_lib');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(!id||!path||!uploadSession)return res.status(400).json({ok:false,error:'Faltan datos de la carga.'});
    if(!verifyUploadSession(uploadSession,id,path))return res.status(401).json({ok:false,error:'La carga venció o no es válida.'});
    ...
    const infoRes=await fetch(`${url}/storage/v1/object/info/consultas-audio/${encodedPath}`,{headers:supabaseHeaders()});
    if(!infoRes.ok)return res.status(409).json({ok:false,error:'El archivo de audio todavía no está disponible.'});

    const row={
      id,tipo:'audio',nombre:name,empresa:company||null,email:email||null,telefono:phone,
      mensaje:null,audio_path:path,audio_mime_type:mime,duracion_segundos:Math.round(duration),
      estado:'nueva',notas:null,valor_cerrado:null,...trackingFields(req.body?.tracking)
    };
